...
pub fn process_pom(
    ...
    let main_pom_content = download_and_read_pom(config, target_folder, pom_url, repo_name)?;
    ...
    let pom_file_path = Path::new(target_folder).join("pom.xml");
    generate_and_analyze_effective_pom(config, target_folder, versions_keywords, &pom_file_path, repo_name)
}
