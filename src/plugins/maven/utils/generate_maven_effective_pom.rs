...
pub fn generate_maven_effective_pom(pom_file: &str) -> Result<String, Box<dyn Error>> {
    ...
    debug!("Preparing to run Maven effective-pom for file '{}', outputting to '{}'", &pom_file, &output_option);
    ...
    debug!("Running 'mvn help:effective-pom' for POM file '{}'", &pom_file);
    ...
    if output.status.success() {
        debug!("Effective POM generated successfully at '{}'", &effective_pom_file);
